...
- **The two Apps swap them through our backend.** The connection-setup messages contain your **public and local IP addresses and ports**, your Checkpoint64 user ID, and the public keys the two Apps use to set up the connection's encryption. Our backend passes them only between members of the same team, and only while one of them is hosting there. It keeps each message only until the other App collects it, usually within a second or two, and deletes any message that is never collected within about 90 seconds.
